"""Tests for ACP Registry metadata shipped with AVA."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "ava-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "AVA"
    assert data["description"]
    assert data["repository"] == "https://github.com/steve2er0/ava"
    assert data["website"].startswith("https://github.com/steve2er0/ava")
    assert data["authors"] == ["Stephen Wells"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"ava-agent[acp]=={data['version']}"
    assert uvx["args"] == ["ava-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"ava-agent[acp]=={_pyproject_version()}"
    )
