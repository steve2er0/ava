...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `ava` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `ava-{profile}` isolates memory per active AVA profile. Empty placeholders collapse cleanly (e.g. `ava-{user}` with no user becomes `ava`). |
...
| `retain_context` | `conversation between AVA and the User` | Context label for retained memories |
