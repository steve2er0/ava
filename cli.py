...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        content_lines = [f"{agent_name} - AI Agent Framework"]
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"AVA v{_version} ({_release_date})"
    else:
    ...
    # Truncate and center to fit.
    lines = []
    for line in content_lines:
        centered = line[:content_width].center(content_width)
        lines.append(
            f"[bold {border_color}]║[/] [{title_color}]{centered}[/] [bold {border_color}]║[/]"
        )
    line2 = version_line[:content_width].center(content_width)
    lines.append(
