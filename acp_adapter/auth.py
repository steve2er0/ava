...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for AVA.
    ...
    not have AVA provider credentials configured yet, so AVA always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate AVA using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure AVA provider",
            description=(
                "Open AVA's interactive model/provider setup in a terminal. "
                "Use this when AVA has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
